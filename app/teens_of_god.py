...
if (dayy=="Mon" and day.x[0]==0):
    ...
    att2=pd.read_csv("web/"+str(dd)+dat[3:8]+".csv")
    print(str(dd)+dat[3:8])
    for i in range(len(att2.index)):
        att.loc[len(att.index)]=list(att2.iloc[i,0:2])+[0,0,0,0,0,0,0]
    att.to_csv("web/"+str(d)+str(dat)+".csv",index=False)
    day.x[0]=1
    day.to_csv("web/day.csv",index=False)
elif(dayy=="Sun"):
    day.x[0]=0
    day.to_csv("web/day.csv",index=False)
...
def add_vol():
    ...
    while(vstt):
            ...
            if event.type==pg.MOUSEBUTTONDOWN:
                if event.button==1:
                    ...
                    if creatr.collidepoint(event.pos):
                        ...
                        qrcod.png("web/data/qrcoda.png",scale=6)
                        volu.loc[len(volu.index)]=[tbox[0].text,tbox[1].text,tbox[2].text,tbox[3].text]
                        att.loc[len(att.index)]=[tbox[0].text,"2022"+str(day.num[0]),0,0,0,0,0,0,0]
                        print(tbox[0].text,tbox[1].text,tbox[2].text,tbox[3].text)
                        volu.to_csv("web/volunterr.csv",index=False)
                        att.to_csv("web/"+dat+".csv",index=False)
                        warnn=base_font.render("New volunteer created succesfully. Click the qrcode to continue",True,green,None)
                        warn=-1
                        warnnr.center=(190,500)
                        idcard = cv2.imread("web/logo.png")
                        font = cv2.FONT_HERSHEY_SIMPLEX
                        cv2.putText(idcard, f'Name: {tbox[0].text}', (10,250), font, 1, (0,0,0), 1, cv2.LINE_AA)
                        cv2.putText(idcard, f'Contact: {tbox[2].text}', (10,300), font, 1, (0,0,0), 1, cv2.LINE_AA)
                        cv2.putText(idcard, f'Number: 2022{list(day.num)[0]}', (10,350), font, 1, (0,0,0), 1, cv2.LINE_AA)
                        x_offset=100
                        y_offset=400
                        qrcod=cv2.imread("web/data/qrcoda.png")
                        idcard[y_offset:y_offset+qrcod.shape[0], x_offset:x_offset+qrcod.shape[1]] = qrcod
                        
                        cv2.imwrite(f"web/data/2022{list(day.num)[0]}.png", idcard)
                        idcard=pg.image.load(f"web/data/2022{list(day.num)[0]}.png")
                        idcard=pg.transform.rotozoom(idcard,0,0.5)
                        idcardr=idcard.get_rect()
                        idcardr.center=(350,200)
                        print("done successfully")
                    try:
                        if idcardr.collidepoint(event.pos):
                            warn=0
                            vstt=False
                    except:
                        continue
        ...
        display.blit(password,passwordr)
        pg.display.flip()
        clock.tick(30)
            ...
            if event.type==pg.MOUSEBUTTONDOWN:
                if event.button==1:
                    ...
                    if newvolr.collidepoint(event.pos):
                        ...
                        display.fill(white)
                        cv2.imwrite("web/data/qrcoda1.png",image)
                        display=pg.display.set_mode((700,700))
                        pg.display.set_caption("Teens of God")
                        try:
                            image=cv2.imread("web/data/qrcoda1.png")
                            detect=cv2.QRCodeDetector()
                            image, points, straight_qrcode = detect.detectAndDecode(image)
                            print(image)
                            att=pd.read_csv("web/"+dat+".csv")
                            email_ind=list(volu["email"]).index(image)
                            att.iloc[email_ind,alpha.index(dayy)+2]=1
                            att.to_csv("web/"+dat+".csv",index=False)
                            UID=list(volu["email"])
                            if(image in UID):
                                email_ind=UID.index(image)
                                num=att.iloc[email_ind,1]
                                found=base_font.render(f"Number {num} marked present",True,green,blue)
                            got=120
                        except:
                            got=120
