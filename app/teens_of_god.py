...
import png
import pyqrcode
from cv2 import flip,rotate,ROTATE_90_COUNTERCLOCKWISE,imread,imwrite,QRCodeDetector,ROTATE_90_CLOCKWISE
import datetime
...
def add_vol():
    ...
    while(vstt):
            ...
            if event.type==pg.MOUSEBUTTONDOWN:
                if event.button==1:
                    ...
                    if creatr.collidepoint(event.pos):
                        ...
                        idcard=pg.Surface((300,400))
                        idcardr=idcard.get_rect()
                        idcardr.center=(350,250)
                        name=base_font.render(f"Name: {tbox[0].text}",True,blue,None)
                        contactt=base_font.render(f"Contact: {tbox[2].text}",True,blue,None)
                        email=base_font.render(f"Number: 2022{str(day.num[0])}",True,blue,None)
                        day.num[0]+=1
                        day.to_csv("web/day.csv",index=False)
        ...
        if(warn==-1):
            display.blit(warnn,warnnr)
            idcard.fill(green)
            idcard.blit(name,namer)
            idcard.blit(contactt,contacttr)
            idcard.blit(email,emailr)
